...
def mapapp(request):
    ...
    if request.method == "POST":
        ...
        osmPath = os.path.join(settings.BASE_DIR, "mapvis/oak.osm")
        ...
        if searchAlgorithm == "Dijkstra":
            shortestPath = dijkstra(
                adj_list, str(closestNodeToStart.id), str(closestNodeToDestination.id)
            )

        elif searchAlgorithm == "A-Star":
            shortestPath = a_star_search(
                adj_list, closestNodeToStart, closestNodeToDestination, nodes
            )

        elif searchAlgorithm == "Breadth-First Search (BFS)":
            shortestPath = breadth_first_search(
                adj_list, closestNodeToStart, closestNodeToDestination
            )

        elif searchAlgorithm == "Greedy Best-First Search":
            shortestPath = greedy_search(
                adj_list, closestNodeToStart, closestNodeToDestination, nodes
            )

        if len(shortestPath) == 0:
            request.session["message"] = (
                "Sorry Couldn't Find Path. Please try different nodes. The Open Street Map data for Addis Ababa, Ethiopia (Gerji) isn't completely filled with all the nodes as accurately as other modern cities."
            )
            return redirect("/")
        else:
